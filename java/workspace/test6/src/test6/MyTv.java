...
public class MyTv {
	...
	public void setVolume(int volume) {
		...
		if( volume > MAX_VOLUME || volume < MIN_VOLUME || !isPowerOn ) {
			return;
		}
	...
	public void setChannel(int channel) {
		...
		// (재사용성 확보와 중복 개발을 회피하기 위하여ㆍ전체 시스템 차원과 단위 시스템 차원의 공통부분을 식별하여 이에 대한 상세 명세를 작성할 수 있다.5)
		if( channel > MAX_CHANNEL || channel < MIN_CHANNEL || !isPowerOn ) {
			return;
		}
	...
	public void gotoPrevChannel() {
		...
		if( this.prevChannel < this.MIN_CHANNEL ) {
			return;
		} else {
			
			int tmp = 0;
			
			tmp = this.prevChannel;
			this.prevChannel = this.channel;
			this.channel = tmp;
		}
